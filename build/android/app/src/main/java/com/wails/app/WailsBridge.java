...
import com.meponder.hyperiondownloader.BuildConfig;
import com.meponder.hyperiondownloader.MainActivity;
...
public class WailsBridge {
     ...
     * Automatically handles "file://" URLs as actual file shares via FileProvider.
     */
        ...
        mainHandler.post(() -> {
            try {
                ...
                if (url != null && url.startsWith("file://")) {
                    //Print that we're handling a file share
                    send.setAction(Intent.ACTION_SEND);
                    
                    // Extract the raw absolute path from the file:// URL
                    String filePath = Uri.parse(url).getPath();
                    File file = new File(filePath);
                    
                    if (file.exists()) {
                        // Generate a secure content:// URI using your app's existing FileProvider
                        ...
                        // Attach the secure URI as a data stream
                        send.putExtra(Intent.EXTRA_STREAM, fileUri);
                        
                        // Explicitly grant temporary read access to whichever app handles the chooser
                        send.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
                        
                        // Determine mime type dynamically based on the file name
                        ...
                        Log.e(TAG, "Share failed: File does not exist at " + filePath);
                        return;
                    }
                    
                    // If text was also provided, include it alongside the file
                    if (!text.isEmpty()) {
                        send.putExtra(Intent.EXTRA_TEXT, text);
                    }
                    
                } else {
                    // Fallback: Standard text/link sharing if URL is not a local file
    ...
    public String getStorageJson() {
        try {
            ...
            String internalStorage = activity.getFilesDir() != null ? activity.getFilesDir().getAbsolutePath() : "";
            String internalCache = activity.getCacheDir() != null ? activity.getCacheDir().getAbsolutePath() : "";
            String externalStorage = activity.getExternalFilesDir(null) != null ? activity.getExternalFilesDir(null).getAbsolutePath() : "";
            return new JSONObject().put("free", free).put("total", total).put("internalStorage", internalStorage != null ? internalStorage : "")
                    .put("internalCache", internalCache).put("externalStorage", externalStorage).toString();
        } catch (Exception e) {
